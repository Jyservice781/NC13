...
// 기능을 따로 관리하고
// 그리고 struct 를 만들어서 데이터를 따로 관리하였다
...
// 특히 static 메서드는 메인 메소드, ScannerUtil 외에는 사용하지 않는다.

// 왜냐하면 static 변수는 프로그램의 생명주기 동안 메모리에 상주하므로,
// 불필요한 메모리 사용을 초래할 수 있기 때문입니다.
// 이는 특히 메모리 자원이 제한적인 환경에서 문제가 될 수 있습니다.

// 생성자 (Constructor)
// 생성자란, 해당 클래스 변수가 초기화 될때 호출되는 메소드로써,
// 처음 해당 클래스 변수에 들어갈 외부의 값이나 아니면 초기화 되는 값
// 또는 특정한 작업을 할 때 호출되는 특수한 메소드이다.
// 만약 우리가 아무런 생성자를 만들어 주지 않는다면
// 자바에서 기본적으로 제공되는 생성자가 호출이 되는데.
// 해당 생성자는 필드의 값을
// 기본형 데이터 타입의 필드 => 0 으로
// 참조형 데이터 타입의 필드 => null 로 초기화해준다.

// 생성자의 경우, 우리가 따로 만들어 줄 수 있는데,
// 한가지 주의할 점은 우리가 파라미터가 있는 생성자만 만들어 주면
// 더 이상 파라미터가 없는 생성자는 호출할 수 없다.
// 만약 파라미터 있는 생성자와 파라미터 없는 생성자 2개가 모두 필요하면
// 둘 다 만들어 주면 된다!
