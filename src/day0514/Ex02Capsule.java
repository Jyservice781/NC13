...
// 캡슐화
// 캡슐화란, 우리가 만든 클래스에 필드를
// 외부에서 직접적으로 접근하지 못하게 막고
// 메소드의 경우, 외부에서 사용하지 않은 메소드이면
// 접근을 막게 만드는 방법이다.
// 캡슐화를 하는 이유는, 우리가 객체 지향적 관점에서
// 외부가 우리 클래스의 필드 혹은 메소드를 직접적으로 값을 넣거나 호출하지 않고
// 객체 그 자체를 소화시키도록 만들기 위함이다.

// 캡슐화의 기본 방법
// 1. 필드는 모두다 private
//    단, private 으로 지정된 필드의 경우 우리가 직접적으로 접근할 수 없기 때문에
//    값을 저장하는 setter 메소드와 저장된 값을 불러오는 getter 메소드를 통하여
//    간접적으로 접근하게 된다.
// 2. 외부에서 쓰는 메소드만 public, 그 외는 모두 다 private

// getter 메소드 만드는 방법
// public 필드타입 get필드이름() {
//      return 필드이름;
//      get필드이름은 carmelCase로 작성
// }
